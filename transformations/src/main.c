...
void apply_spin(struct shader const* s)
{
    // Create matrix that will be our translation matrix.
    // Initialized to identity matrix
    mat4 trans = GLM_MAT4_IDENTITY_INIT;

    vec3 translate_vector = { 0.5f, -0.5f, 0.0f };
    glm_translate(trans, translate_vector);

    // Rotate 90 deg around z-axis
    vec3 axis = { 0.0f, 0.0f, 1.0f };
    glm_rotate(trans, glfwGetTime(), axis);

    // Pass to uniform of vertex shader
    unsigned int transform_loc = glGetUniformLocation(s->ID, "transform");
